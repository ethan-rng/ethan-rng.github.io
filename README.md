...
Experience has no route of its own, it's a section on the landing page,
rendered straight from `data/experience.ts`.
...
on mobile. `size: "wide"` makes a tile span two columns, currently the first
project as a feature tile. Wide tiles get a 16:6 image, single cells get 3:2.

The five entries fill exactly two rows at `lg` and three at `sm` with no
gaps. Adding or removing one leaves the last row ragged unless you adjust a
`size`.

The two Hack Western entries are real; the rest are placeholders using
generated SVGs in `public/images/projects/`. Drop a real image in there and
point `image` at it. `heroImage` is optional and overrides `image` at the top
of the case study, for when the tile art and the banner shot differ.

### Images

Everything photographic is grayscale at rest and comes to colour on hover,
including the case study and post banners. That one rule covers the avatar,
the About walls, the experience marks and the banners.

| Where | Source |
| ----- | ------ |
| `public/images/avatar.jpg` | sidebar circle, via `site.avatar` |
| `public/images/portrait.jpg` | landing hero, via `site.portrait` |
| `public/images/elsewhere/` | the About travel wall, `elsewherePhotos` |
| `public/images/food/` | the About food wall, `foodPhotos` |
| `public/images/experience/` | company logos, one per role |
| `public/images/projects/` | tile art and case study banners |

The experience marks are real logos (Meta, MLH, Ivey) rendered on a light
chip with `object-contain`. That's deliberate: Ivey's dark green and MLH's
navy are invisible against a near-black tile, and a wide wordmark would be
cropped to its middle by `object-cover`.

Sources are sized for how they actually render, roughly 2x the display size
and no more. Passing `next/image` a width far larger than the rendered box
makes it generate and serve a needlessly large rendition.
...
| `highlights` | numbered 01/02/03 cards, under their own heading   |
| `sections`   | long-form `heading` + `paragraphs`, in order      |
| `heroImage`  | optional banner, overrides `image` at the top     |

The placeholder projects share one `placeholderCaseStudy` object, so editing
it changes all of them at once; split it up as you write real ones.

A note on house style: no em dashes anywhere in the copy. Use a colon, a
semicolon, parentheses, or two sentences.
...
| Route              | Holds                                          |
| ------------------ | ---------------------------------------------- |
| `/`                | the ASCII hero, three featured projects, experience |
...
app/
  ...
  page.tsx        landing page, hero, selected work, experience
  about/          bio + photos
  work/           projects index
